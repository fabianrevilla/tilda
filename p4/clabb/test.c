...
int main() {
	...
	for (long i = 0; i < HASHVEKSIZE; i++) {
		...
		// spara hashtabellens indexvärde och antalet kollisioner till kollisioner.dat
		fprintf(fh, "%ld\t%ld\n", i, collisions_bucket);
	...
	for (long n = 10000; n < 1000000; n += 10000) {
		...
		// spara n och hur lång tid det tog att göra n hämtningar till uppslagning.dat
		fprintf(fh, "%ld\t%lf\n", n, time);
	}
	fclose(fh);
